...
namespace MetisPlaywright.Tests
{
    public class _01_Setup_Data : BaseTest
    {
        ...
        public async Task T05_Setup_CreateAssetType_AutoTests1()
        {
            ...
            Thread.Sleep(2000); // Wait for the grid to load
        ...
        public async Task ActivateOptimizationForAutoTestsCompany(string companyUUID)
        {
            ...
            await using var command = new NpgsqlCommand(query, connection);
            command.Parameters.AddWithValue("companyUUID", Guid.Parse(companyUUID));
            command.Parameters.AddWithValue("createdAt", new DateTimeOffset(2026, 5, 1, 0, 0, 0, TimeSpan.Zero));
